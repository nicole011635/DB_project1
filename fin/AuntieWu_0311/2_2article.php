...
if($_SERVER["REQUEST_METHOD"] == "POST"){
	...
	$sql='SELECT * FROM `news` WHERE `nTitle`="'.$nTitle.'"';
}//end if
		...
		<div class="col-md-10">
			<ul class="breadcrumb">
				...
				<li>
					...
					<a href="2_news.php">最新消息</a> <span class="divider"></span>

				</li>
				<li>

					<a href="#"><?PHP echo"$nTitle";?></a> <span class="divider"></span>
	...
	$row=mysqli_fetch_array($result);
  	...
  	echo '
			<div class="row"  style="margin-left:2%;">
				<div style="border-width:1px; border-style:solid;padding:4%;">
					<h2>'.$row["nTitle"].'</h2>
					<h4>'.$row["nTime"].'</h4>
					<br><br>
					<p>'.$row["nContent"].'
					...
					<div style="text-align:center;">
						<br><br>
						<img width="80%" src="'.$row["nPic"].'"><br><br><br><br>
						<a href="2_news.php" class="btn btn-default"> 回上頁</a>
					</div>					
				</div>
			</div>
...
$sql='SELECT * FROM `news` ORDER BY `nId` DESC LIMIT '.$set.', 3';
...
if ($result=mysqli_query($con,$sql)){
	...
	while($row=mysqli_fetch_array($result)){
		...
		<div class="thumbnail">
					...
					<img alt="newsPic" src="'.$row["nPic"].'" width="200px" height="200px">

					<p style="padding-left:15%;">'

						.mb_substr($row['nContent'], 0,30,"UTF-8").

						//在UTF-8的編碼下，因為中文字為3位元組，只能echo mb_substr($content,0,15,"UTF-8")不然會出現亂碼;

					'......</p>

					<p>

						<a  id="modal-10335" href="#modal-container-10335" role="button" class="btn" data-toggle="modal">繼續閱讀 »</a>
...
?>
				...
				<form action="2_news.php" method="post">
	...
	for($i=1;$i<=$totalpages;$i++){
		...
		echo '<li>
			...
			<a><input type="submit" name="currentpage" class="pageBtn" value="'.$i.' "></a>
